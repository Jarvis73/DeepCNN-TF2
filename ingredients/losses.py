...
def add_arguments(parser: argparse.ArgumentParser):
    parser.add_argument_group("Loss Arguments")
    parser.add_argument("-l", "--loss", type=str, default="scce",
                        help="Loss used for training. [lda/scce]")
...
class Loss(object):
    def __init__(self, cfg):
        if cfg.loss == "lda":
            self.loss = LinearDiscriminativeLoss()
        elif cfg.loss == "scce":
            self.loss = K.losses.SparseCategoricalCrossentropy(from_logits=True)
        else:
            raise ValueError(f"Unsupported loss: {cfg.loss}. [lda/scce]")
